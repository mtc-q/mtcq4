...
// Base path of the app (handles GitHub project pages like /repo-name/)
function basePath() {
  return window.location.pathname.replace(/[^/]*$/, "");
}

/** Public URL for a tree — pretty /username path where a 404/rewrite trick exists,
 *  otherwise the always-works query form. Adapts to whatever host the site is on. */
export function treeUrl(username) {
  const { origin, protocol, hostname } = window.location;
  const pretty = protocol === "https:" &&
    (hostname.endsWith(".github.io") || hostname.endsWith(".web.app") ||
     hostname.endsWith(".firebaseapp.com") || !hostname.includes("localhost"));
  if (pretty && protocol === "https:") return `${origin}${basePath()}${username}`;
  return `${origin}${basePath()}tree.html?u=${username}`;
}

/** The link shown to users — the real current host, e.g. "you.github.io/mtcq4/name". */
export function displayUrl(username) {
  return `${window.location.host}${basePath()}${username}`;
}
...
export const WALLPAPERS = {
  ...
  sunset:   { label: "Sunset",   css: "linear-gradient(160deg,#ff5c1a 0%,#ff9d5c 100%)",      text: "#ffffff" },
  peach:    { label: "Peach",    css: "linear-gradient(160deg,#fff1e9 0%,#ffd9c4 100%)",      text: "#1a1512" },
...
export function say(el, text, ok = true) {
  el.textContent = text;
  el.style.borderLeftColor = ok ? "#ff5c1a" : "#b9b0a5";
